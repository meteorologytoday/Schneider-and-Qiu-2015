...
struct BasicMatrixOperators
    ...
    function BasicMatrixOperators(;
        ...
        T[:, 1:Ny-1, :] = num_V[:, 2:Ny, :]; T[:, Ny, :] = num_V[:, 1, :];  T_S_V = build!(V_I_V_expand, T);
        T[:, :     , :] = num_V[:, 1:Ny, :];                                T_N_V = build!(V_I_V_expand, T);

        V_S_T = T_N_V'
        V_N_T = T_S_V'
        #V[:, 1:Ny, :]    = num_T;                                           V_S_T  = build!(T_I_T_expand, V);
        #V[:, 2:Ny+1, :]  = num_T;                                           V_N_T  = build!(T_I_T_expand, V);
        ...
        T[:, :, 1:Nz-1] = view(num_T, :, :, 2:Nz);    T_DN_T = build!(T_I_T_expand, T; wipe=:b)
        T[:, :, 2:Nz  ] = view(num_T, :, :, 1:Nz-1);  T_UP_T = build!(T_I_T_expand, T; wipe=:t)

        T[:, :, :] = view(num_W, :, :, 2:Nz+1);       T_DN_W = build!(W_I_W_expand, T)
        T[:, :, :] = view(num_W, :, :, 1:Nz);         T_UP_W = build!(W_I_W_expand, T)
        ...
        T[1:Nx-1, :, :] = num_U[2:Nx, :, :];  T[Nx, :, :] = num_U[1, :, :];   T_W_U = build!(U_I_U_expand, T);
