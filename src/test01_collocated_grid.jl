...
op = (
     ...
     + amo_slb.T_interp_V * v_0_mean * amo_slb.V_∂y_T
...
op_cont = sparse((
      ...
      * [ spzeros(Float64, T_pts, T_pts + T_pts + W_pts)   repeat(amo_slb.U_∂x_T, outer=(gd.Nz, 1)) ; 
      ...
      + [ spzeros(Float64, T_pts, T_pts + T_pts)  (h_0*amo.T_DIVz_W)  spzeros(Float64, T_pts, sT_pts) ]
...
op_mom_damp = 0*[ blockdiag( (10800.0)^(-1) * amo.bmo.T_I_T, (10800.0)^(-1) * amo.bmo.T_I_T)    spzeros(2*T_pts, W_pts + sT_pts) ]
...
op_mom = op_mom_adv + op_mom_vadv + op_mom_cori + op_mom_vdif + op_mom_reduced_gravity + op_mom_damp
#op_mom = op_mom_adv +  op_mom_cori + op_mom_vdif + op_mom_reduced_gravity
#op_mom = op_mom_adv + op_mom_reduced_gravity
...
#RHS = [ (RHS_mom_pt + RHS_mom_vdif_bc) ; zeros(Float64, T_pts) ; ]
...
filename = "test_solution_ysym.nc"
...
ax[3].set_xlabel("u_0 [m / s]")
ax[3].set_ylabel("v_g [m / s]")
