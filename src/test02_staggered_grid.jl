...
op_cont = sparse((
      ...
      * [ spzeros(Float64, T_pts, U_pts + V_pts + W_pts)   repeat(amo_slb.U_∂x_T, outer=(gd.Nz, 1)) ; 
          spzeros(Float64, T_pts, U_pts + V_pts + W_pts)   repeat(amo_slb.V_∂y_T, outer=(gd.Nz, 1)) ; ]
      + [ spzeros(Float64, T_pts, U_pts + V_pts)  amo.T_DIVz_W  spzeros(Float64, T_pts, sT_pts) ]
      - [ spzeros(Float64, T_pts, U_pts + V_pts + W_pts)   h_diffusivity * ( repeat(amo_slb.T_DIVx_U * amo_slb.U_∂x_T + amo_slb.T_DIVy_V * amo_slb.V_∂y_T, outer=(gd.Nz, 1)) ) ]
))
