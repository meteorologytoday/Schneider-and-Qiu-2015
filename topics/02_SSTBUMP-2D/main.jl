...
@everywhere output_dir = "output_strong_E0"

mkpath(output_dir)


Ugs = [15.0,]#collect(Float64, 4:4:20)
Vg = 0.0

ΔSSTs = collect(Float64, range(0, 1, length=11))

#ΔSSTs = [1.0]

#@distributed for Ug in Ugs, ΔSST in ΔSSTs
@sync @distributed for (Ug, ΔSST) in collect(Iterators.product(Ugs, ΔSSTs))
    
    output_filename = format("Ug_{:.1f}-dSST_{:.2f}.nc", Ug, ΔSST)
    full_filename = joinpath(output_dir, output_filename) 

    printfmtln("Doing the case: (Ug, ΔSST) = ({:f}, {:f}). Output: {:s}", Ug, ΔSST, output_filename)
